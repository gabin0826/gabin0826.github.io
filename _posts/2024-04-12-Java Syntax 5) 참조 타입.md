...
- **new 연산자를 이용**
  ...
  `String name1 = new String("gabin");`   `String name2 = new String("gabin");` 이렇게 되면 String 객체가 각각 새롭게 만들어지기 때문에 같은 값을 참조하지 않는다. 중복으로 데이터(객체)가 생성된다는 것이다

❗️ **그러므로 new 연산자를 사용하는 것과 그냥 대입 선언 방식은 완전히 다른 **
