...
class Car{ // 라이브러리 클래스
 int speed = 80;
 boolean start = true;
 String model; //null -> 필드의 초기값이 지정되지 않으면 초기값이 자동으로 들어간다
}
```
<br>
<br>

**필드 사용** : 필드를 읽고 변경하는 것 필드를 선언했다고 바로 사용할 수 있는게 아니다
필드는 객체의 데이터이므로 데이터가 객체 안에 존재해야한다
...
```java
void method(..) { 
	speed = 30; //참조 변수와 도트 연산자가 없다면 객체 내의 메소드에서 사용하는 것!!!!
}
```
<br>
...
필드는 클래스블록에서 선언, 객체 내부에 존재하고 객체 내외부 생성 사용 O -> 메서드가 필요하지 않다
